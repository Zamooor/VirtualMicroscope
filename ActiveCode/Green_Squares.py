...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def makeObject(self):
        ...
        for z in range(0, 30):
            ...
            x = (randint(0,100) - 50.0) / 50.0
            y = (randint(0,100) - 50.0) / 50.0
            self.algaeList.append(algaes(x,y))
            self.Create_Square_2(x, y, 0.01,self.trolltechGreen)
            GL.glEnd()
            self.algaeList[-1].object=genList
            self.algaeList[-1].posY= y
            self.algaeList[-1].posX= x
            print genList
    ...
    def Create_Square_2(self, x, y, size, color):
        self.qglColor(color)
        #bindTexture ("/testpic2")
        GL.glTexCoord2d(1,1); GL.glVertex2d(x + size, y + size);
        GL.glTexCoord2d(0,1); GL.glVertex2d(x + size, y - size);
        GL.glTexCoord2d(1,0); GL.glVertex2d(x - size, y - size);
        GL.glTexCoord2d(0,0); GL.glVertex2d(x - size, y + size);
